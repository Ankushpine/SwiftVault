...
const Sidebar = ({ isDark, activeGroup, groups, isLoading, isOpen = false, onClose, onGroupChange, onEditGroup, onDeleteGroup }: SidebarProps) => {
    ...
    const sidebarContent = (
        <div className={`w-52 h-full flex flex-col border-r transition-colors ${isDark ? 'bg-[#0a0e14] border-gray-800' : 'bg-white border-gray-200'}`}>
            {/* Logo */}
            <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className={`font-bold text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>Vault</span>
                </div>
                {/* Close button - mobile only */}
                <button
                    onClick={onClose}
                    className={`md:hidden p-1.5 rounded-lg transition-colors ${isDark ? 'hover:bg-gray-800 text-gray-400' : 'hover:bg-gray-100 text-gray-500'}`}
                ...
                <div className="space-y-1">
                    {menuItems.map((item) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${activeGroup === item.id
                                ? isDark
                        ...
                        {isLoading ? (
                            ...
                            groups.map((group) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${activeGroup === group.id
                                        ? isDark
                                            ? 'bg-blue-600/20 text-blue-400'
                                            : 'bg-blue-50 text-blue-600'
                                        : isDark
                                ...
                                >
                                    ...
                                    <div className="relative group/btn" onClick={(e) => e.stopPropagation()}>
                                        <button
                                            ...
                                            className={`p-1 rounded transition-colors ${isDark ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-200 text-gray-500'} ${openDropdown === group.id ? 'bg-gray-200 dark:bg-gray-700' : ''}`}
                                        >
