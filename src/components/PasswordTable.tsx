...
const PasswordTable = ({ isDark, passwords, groupName, isLoading, onEdit, onDelete, onToggleFavorite, onViewDetails }: PasswordTableProps) => {
    ...
    const getInitials = (account: string): string => account.charAt(0).toUpperCase()
    ...
    const getColorForAccount = (account: string): string => {
        ...
        return colors[account.charCodeAt(0) % colors.length]
    }
    ...
    const MobileSkeletonCard = () => (
        <div className={`p-4 rounded-xl border animate-pulse ${isDark ? 'bg-[#1a1f2e] border-gray-800' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`} />
                <div className="flex-1">
                    <div className={`h-4 rounded w-32 mb-2 ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`} />
                    <div className={`h-3 rounded w-20 ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`} />
                </div>
            </div>
        </div>
    )
    ...
    const MobileCard = ({ password }: { password: Password }) => (
        <div
            onClick={() => onViewDetails?.(password)}
            className={`p-4 rounded-xl border cursor-pointer transition-colors ${isDark ? 'bg-[#1a1f2e] border-gray-800 hover:bg-[#0f1419]' : 'bg-white border-gray-200 hover:bg-gray-50'}`}
        >
            <div className="flex items-center gap-3">
                {/* Avatar */}
                <div className={`w-10 h-10 rounded-full ${getColorForAccount(password.account)} flex items-center justify-center text-white font-semibold text-sm flex-shrink-0`}>
                ...
                <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                        ...
                        className={`p-2 rounded-lg transition-all ${password.isFavorite ? 'text-yellow-500' : isDark ? 'text-gray-600' : 'text-gray-400'}`}
                    >
                        {password.isFavorite ? (
                    ...
                    <div className="relative">
                        <button
                            onClick={() => toggleDropdown(password.id)}
                        ...
                        {openDropdown === password.id && (
                            <div
                                ref={dropdownRef}
                                className={`absolute right-0 mt-1 w-40 rounded-lg shadow-lg z-20 border ${isDark ? 'bg-[#1a1f2e] border-gray-700' : 'bg-white border-gray-200'}`}
                            >
                                <button
                                    onClick={() => handleEdit(password)}
                                    className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800' : 'text-gray-700 hover:bg-gray-50'}`}
                                >
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <button
                                    ...
                                    className={`w-full px-4 py-2.5 text-left text-sm flex items-center gap-2 transition-colors ${isDark ? 'text-red-400 hover:bg-gray-800' : 'text-red-600 hover:bg-gray-50'}`}
                                >
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className={`mt-3 flex items-center gap-2 text-xs font-mono ${isDark ? 'text-gray-500' : 'text-gray-400'}`} onClick={(e) => e.stopPropagation()}>
                <span className="flex-1 truncate">
                    {visiblePasswords.has(password.id) ? password.password : '••••••••'}
                </span>
                {password.password && (
                    <>
                        <button onClick={() => togglePasswordVisibility(password.id)} className={`p-1 rounded ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-200'}`}>
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                        </button>
                        <CopyButton text={password.password} fieldId={`m-password-${password.id}`} />
                    </>
                )}
            </div>
        </div>
    ...
    return (
        <div className={`h-full flex flex-col rounded-xl overflow-hidden transition-colors border ${isDark ? 'bg-[#1a1f2e] border-gray-800' : 'bg-white border-gray-200'}`}>
            {/* Count Header */}
            <div className={`px-4 sm:px-6 py-3 sm:py-4 border-b flex-shrink-0 ${isDark ? 'border-gray-800 bg-[#0f1419]' : 'border-gray-200 bg-gray-50'}`}>
            ...
            <div className="md:hidden flex-1 overflow-y-auto p-3 space-y-2">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 gap-3">
                        ...
                        <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>No passwords yet</p>
                    </div>
                ) : (
                    passwords.map(password => <MobileCard key={password.id} password={password} />)
                )}
            </div>
                ...
                <div className="flex-1 overflow-auto">
                    <table className="w-full">
                        <thead className={`sticky top-0 z-10 ${isDark ? 'bg-[#0f1419]' : 'bg-gray-50'}`}>
                            <tr>
                                {['Account', 'Username', 'Email', 'Password', 'Phone No.', 'Security Details', 'Favorite', ''].map((h, i) => (
                                    <th key={i} className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider ${i === 6 ? 'text-center' : ''} ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                        {h}
                                    </th>
                                ))}
                            </tr>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{password.account}</span>
                                            </div>
                                        </td>
                                                ...
                                                <div className="flex items-start gap-2" onClick={(e) => e.stopPropagation()}>
                                                        ...
                                                        {password.securityDetails.split('\n').map((line: string, i: number) => {
                                                            ...
                                                            return (
                                                                <div key={i} className="flex items-center gap-2">
                                                                    ...
                                                                    <button onClick={() => toggleSecurityVisibility(password.id)} className={`p-0.5 rounded ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                                    ...
                                                    <CopyButton
                                                        text={(() => {
                                                            ...
                                                            const answerLine = lines.find((line: string) => !line.startsWith('Q:'))
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); onToggleFavorite?.(password) }}
                                                className={`p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-700 transition-all ${password.isFavorite ? 'text-yellow-500' : isDark ? 'text-gray-400' : 'text-gray-500'}`}
                                            >
                                                {password.isFavorite ? (
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right relative">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); toggleDropdown(password.id) }}
                                            ...
                                            {openDropdown === password.id && (
                                                ...
                                                >
                                                    <div className="py-1">
                                                        <button
                                                            onClick={() => handleEdit(password)}
                                                            className={`w-full px-4 py-2 text-left text-sm flex items-center gap-2 transition-colors ${isDark ? 'text-gray-300 hover:bg-gray-800' : 'text-gray-700 hover:bg-gray-50'}`}
                                                        >
                                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                            </svg>
                                                        ...
                                                        <button
                                                            onClick={() => handleDelete(password)}
                                                            className={`w-full px-4 py-2 text-left text-sm flex items-center gap-2 transition-colors ${isDark ? 'text-red-400 hover:bg-gray-800' : 'text-red-600 hover:bg-gray-50'}`}
                                                        >
                                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                            </svg>
